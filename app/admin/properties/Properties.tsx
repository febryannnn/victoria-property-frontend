...
export default function PropertiesPage() {
    ...
    function mapProperties(data: any[]): Property[] {
        return (data || []).map((item: any) => ({
            ...
            created_at: item.created_at, cover_image_url: item.cover_image_url,
            property_type_id: item.property_type_id, user_id: item.user_id,
            latitude: item.latitude ?? 0,   // ← add this
            longitude: item.longitude ?? 0, // ← add this
        }));
    }
    ...
    const handleCreateProperty = async (property: Property): Promise<Property> => {
        const result = await createProperty(property);
        await fetchProperties();
        return result; // createProperty already returns Promise<Property>
    };

    const handleUpdateProperty = async (property: Property): Promise<Property> => {
        const result = await updateProperty(selectedProperty!.id!, property);
        await fetchProperties();
        return result;
    };
